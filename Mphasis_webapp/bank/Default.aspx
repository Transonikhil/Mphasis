<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="Mphasis_webapp.bank.Default" %>
                            ...
                            <asp:Chart ID="Chart1" runat="server" 
                                ...
                                <Legends>
                                    <asp:Legend Name="Legend1" Title="Sites Audited / Pending">
                                        ...
                                        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:SQLConnstr %>" SelectCommand="select count(distinct d.atmid) as Audited,'Y' as [Y] from DR_CTP d join atms a on d.atmid=a.atmid where  a.bankid='TAMILNAD MERCANTILE BANK LIMITED' AND DATEPART(mm,vdate)=datepart(mm,CONVERT(VARCHAR(10), GETDATE(), 101))   Union all select case when (select distinct COUNT(atmid) - (select count(distinct atmid) from DR_CTP where DATEPART(mm,vdate)=datepart(mm,CONVERT(VARCHAR(10), GETDATE(), 101))) from ATMs where atmstatus&lt;&gt;'Inactive' )&lt;=0 then 0 else (select distinct COUNT(atmid) - (select count(distinct d.atmid) from DR_CTP d join atms a on d.atmid=a.atmid where  DATEPART(mm,vdate)=datepart(mm,CONVERT(VARCHAR(10), GETDATE(), 101)) ) from ATMs where atmstatus&lt;&gt;'Inactive') end ,'Y'">
