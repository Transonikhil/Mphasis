...
public class ibuckethead
{
    public static GridView Gridview = null; public static Page Page = null;
    public static TextBox TextBox = null; public static string[] return_results = null;
    public static string Query = null;
    public string cleanText(TextBox t)
    {
        if (t.Text == "" || t.Text == null) { t.Text = "NA"; } else { t.Text = t.Text.Trim().Replace("'", ""); } return t.Text;
    }
    ...
    public DataTable BindoboutGrid(string query)
    {
     ...
     public string returnmonth(string vid)
    {
        string month = "";
        if (vid.Contains("-"))
        {
            string[] vi = vid.Split('_');
            month = vi[1].Trim();
        }
        else
        {
    ...
    {
        string date = System.DateTime.Now.ToString("HH:mm:ss-MM'/'dd'/'yyyy");
        string[] lines = { date, error };
        System.IO.File.AppendAllLines(HttpContext.Current.Server.MapPath("~/ErrorLog.txt"), lines);
    }
}