<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="Mphasis_webapp.RM.Default" %>
                ...
                <table class="styleall" width="80%">
                            ...
                            <asp:Chart ID="Chart1" runat="server" 
                                ...
                                <Legends>
                                    <asp:Legend Name="Legend1" Title="Sites Audited / Pending">
                                        ...
                                        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:SQLConnstr %>" SelectCommand="select count(distinct d.atmid) as Audited,'Y' as [Y] from DR_CTP d join atms a on d.atmid=a.atmid where  DATEPART(mm,vdate)=datepart(mm,CONVERT(VARCHAR(10), GETDATE(), 101)) and RM like @rm  Union all select case when (select distinct COUNT(atmid) - (select count(distinct atmid) from DR_CTP where DATEPART(mm,vdate)=datepart(mm,CONVERT(VARCHAR(10), GETDATE(), 101))) from ATMs where atmstatus&lt;&gt;'Inactive')&lt;=0 then 0 else (select distinct COUNT(atmid) - (select count(distinct d.atmid) from DR_CTP d join atms a on d.atmid=a.atmid where DATEPART(mm,vdate)=datepart(mm,CONVERT(VARCHAR(10), GETDATE(), 101)) and RM like @rm) from ATMs where atmstatus&lt;&gt;'Inactive' and RM like @rm) end ,'Y'">
                    ...
                    <tr>
                        <td colspan="2" class="style8">
                            ...
                            <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:SQLConnstr %>" SelectCommand="select u.userid as [area officer],COUNT(d.vid) as [visit] from DR_CTP d, users u where  d.USERID=u.userid and RM like @rm  and convert(datetime,d.vdate ) between CONVERT(VARCHAR(10), dateadd(D, -30, GETDATE()), 101) AND CONVERT(VARCHAR(10), GETDATE(), 101) group by u.userid union all select distinct userid, '0' from users where userid&lt;&gt;'AREA OFFICER' and RM like @rm and role in ('AO','DE') and userid&lt;&gt;'admin' and userid not in (select distinct u.userid from dr_ctp d, users u where  d.userid=u.userid)  order by visit desc">
                                <SelectParameters><asp:SessionParameter Name="rm" SessionField="Sess_username" DefaultValue="%" /></SelectParameters>
                            </asp:SqlDataSource>
                        </td>
                    </tr>
