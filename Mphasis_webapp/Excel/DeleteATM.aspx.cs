...
{
    public partial class DeleteATM : System.Web.UI.Page
    {
                        ...
                        using (var excel = new ExcelPackage(fileuploadExcel.PostedFile.InputStream))
                        {
                            ...
                            for (int rowNum = 2; rowNum <= 10000; rowNum++)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (val != "1")
                                    {
                                        string ins = @"Update atms set atmstatus='Inactive' where atmid='" + AtmId.ToString().Trim().ToUpper() + "'";
                                        int i = obj.NonExecuteQuery(ins);

                                        if (i > 0)
                                        {
                                            string q1 = "Update UserMap set status='DEL',serverstatus='DEL' where atmid='" + AtmId.ToString().Trim().ToUpper() + "'";
                                            obj.NonExecuteQuery(q1);
                                            string q2 = "Update Users set status='MOD', datastatus='MOD' where status<>'DEL' and userid in (Select userid from UserMap where atmid='" + AtmId.ToString().Trim().ToUpper() + "')";
                                            obj.NonExecuteQuery(q2);
                                            add.Rows.Add(AtmId.ToString().Trim().ToUpper(), "Atmid Deleted Successfully");
                                        }
                                        else
                                        {
                                            notadd.Rows.Add(AtmId.ToString().Trim().ToUpper(), "Atmid Not Deleted");
                                        }
                                        //Response.Write(query);
                                    }
                                    else
                                    {
                                        notadd.Rows.Add(AtmId.ToString().Trim().ToUpper(), "Invalid Atmid");
                                    }
                                }
                            }
                            Session["idadd"] = null;
        ...
        {
            string XlsPath = Server.MapPath(@"~/Excel/DeleteATM.xlsx");
        ...
        protected void lnkaddexport_Click(object sender, EventArgs e)
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;filename=deletedAtm.xlsx");
        ...
        protected void lnknotadd_Click(object sender, EventArgs e)
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;filename=NotdeletedAtm.xls");
                Response.Charset = String.Empty;
                Response.ContentType = "application/vnd.xls";
