...
{
    public partial class ExcelMapAtms : System.Web.UI.Page
    {
                    ...
                    try
                    {
                        using (var excel = new ExcelPackage(fileuploadExcel.PostedFile.InputStream))
                        {
                            ...
                            for (int rowNum = 2; rowNum <= 10000; rowNum++)
                            {
                                ...
                                {
                                    SqlCommand cmd = new SqlCommand("SELECT atmid FROM [atms]  WHERE [atmid] = '" + atmid.ToString().Trim() + "'  and atmstatus='Active'", cn);
                                    cn.Open();
                                    ...
                                    {
                                        cn.Close();
                                        SqlCommand cmd2 = new SqlCommand("SELECT userid FROM [users]  WHERE [userid] = '" + userid.ToString().Trim() + "' and status<>'DEL'", cn);
                                        cn.Open();
                                        ...
                                        if (dr2.Read())
                                        {
                                            ...
                                            if (dr1.Read())
                                            {
                                                ...
                                                if (status == "DEL" || status == "SYN")
                                                {
                                                    string update = "update [usermap] set [status]='MOD', [serverstatus]='MOD' WHERE [atmid]='" + atmid.ToString().Trim() + "' AND [userid] = '" + userid.ToString().Trim() + "'";
                                                    string r = "update [users] set status='MOD', datastatus='MOD'  where userid='" + userid.ToString().Trim() + "'";
                                                    int val = obj.NonExecuteQuery(update);
                                                    if (val != 0)
                                                    {
                                                        int val1 = obj.NonExecuteQuery(r);
                                                        if (val1 != 0)
                                                        {
                                            ...
                                            {
                                                cn.Close();
                                                string query = "insert into usermap(userid,atmid,status,serverstatus) values('" + userid.ToString().Trim() + "','" + atmid.ToString().Trim().ToUpper() + "','CRE','CRE')";
                                                int i = obj.NonExecuteQuery(query);
                                                string r = "update [users] set status='MOD', datastatus='MOD'  where userid='" + userid.ToString().Trim() + "'";
                                                if (i != 0)
                                                {
                                                    int j = obj.NonExecuteQuery(r);
                                                    add.Rows.Add(userid.ToString().Trim(), atmid.ToString().Trim());
                                                }
                                                else
                                                {
                                                    notadd.Rows.Add(userid.ToString().Trim(), atmid.ToString().Trim());
                                                }
