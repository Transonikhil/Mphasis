...
namespace Mphasis_webapp.Excel
{
    public partial class DeleteUser : System.Web.UI.Page
    {
                        ...
                        using (var excel = new ExcelPackage(fileuploadExcel.PostedFile.InputStream))
                        {
                            ...
                            for (int rowNum = 2; rowNum <= 10000; rowNum++)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (val != "1")
                                    {
                                        string updatestatus = @"update Users set Status='DEL',datastatus='DEL' where userid='" + AtmId.ToString().Trim() + "'";
                                        int i = obj.NonExecuteQuery(updatestatus);
                                        string usrmp = @"update usermap set Status='DEL',serverstatus='DEL' where userid='" + AtmId.ToString().Trim() + "'";
                                        obj.NonExecuteQuery(usrmp);
                                        ...
                                        if (i >= 0)
                                        {
                                            ...
                                            add.Rows.Add(AtmId.ToString().Trim(), "Userid Deleted Successfully");
                                        }
                                        else
                                        {
                                            notadd.Rows.Add(AtmId.ToString().Trim(), "Userid Not Deleted");
                                        }
                                        //Response.Write(query);
                                    }
                                    else
                                    {
                                        notadd.Rows.Add(AtmId.ToString().Trim(), "Invalid Userid");
                                    }
                                }
                            }
        ...
        {
            string XlsPath = Server.MapPath(@"~/Excel/DeleteUser.xlsx");
        ...
        protected void lnkaddexport_Click(object sender, EventArgs e)
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;filename=DeletedUser.xlsx");
        ...
        protected void lnknotadd_Click(object sender, EventArgs e)
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;filename=NotdeletedUsers.xls");
                Response.Charset = String.Empty;
                Response.ContentType = "application/vnd.xls";
